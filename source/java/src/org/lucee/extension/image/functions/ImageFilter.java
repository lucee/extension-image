...
public class ImageFilter extends FunctionSupport {
	...
	public static String call(PageContext pc, Object name, String filterName, Struct parameters) throws PageException {
		...
		if(clazz==null) {
			...
			throw eng.getExceptionUtil().createFunctionException(pc, "ImageFilter", 2, "filtername", "invalid filter name ["+filterName+"], valid filter names are ["+list+"]",null);
		}
	...
	private static void setters(String key, Class clazz, StringBuilder sb) {
		...
		sb.append("			throw CFMLEngineFactory.getInstance().getExceptionUtil().createFunctionException(eng.getThreadPageContext(), \"ImageFilter\", 3, \"parameters\", \"the parameter\"+(parameters.size()>1?\"s\":\"\")+\" [\"+List.arrayToList(parameters.keys(),\", \")+\"] \"+(parameters.size()>1?\"are\":\"is\")+\" not allowed, only the following parameters are supported ["+names+"]\");\n");
		sb.append("		}\n");
		sb.append("\n");
