...
public class JPEGEncoder {
	...
	public static int[] jpegNaturalOrder = { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49, 56,
	...
	void WriteCompressedData(BufferedOutputStream outStream) throws IOException {
		...
		/*
		 * This method controls the compression of the image. Starting at the upper left of the image, it
		 * compresses 8x8 blocks of data until the entire image has been compressed.
		 */
				...
				for (comp = 0; comp < JpegObj.NumberOfComponents; comp++) {
					...
					for (i = 0; i < JpegObj.VsampFactor[comp]; i++) {
						for (j = 0; j < JpegObj.HsampFactor[comp]; j++) {
							...
							// if ((!JpegObj.lastColumnIsDummy[comp] || c < Width - 1) && (!JpegObj.lastRowIsDummy[comp] || r <
							// Height - 1)) {
...
class DCT {
	 ...
	 * computing the DCT and it's inverse. This also initializes the run length counters and the ZigZag
	 * sequence. Note that the image quality can be worse than 25 however the image will be extemely
	...
	/*
	 * This method preforms a DCT on a block of image data using the AAN method as implemented in the
	 * IJG Jpeg-6a library.
	 */
	...
	int[] quantizeBlock(double inputData[][], int code) {
		...
		for (i = 0; i < 8; i++) {
			for (j = 0; j < 8; j++) {
				...
				// outputData[index] = (int)(((inputData[i][j] * (((double[]) (Divisors[code]))[index])) + 16384.5)
				// -16384);
				index++;
			}
		}
...
class Huffman {
	...
	private int[] valACluminance = { ,
	...
	private int[] valACchrominance = { ,
	 ...
	 */
	private static int[] jpegNaturalOrder = { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49, 56,
			57, 50, 43, 36, 29, 22, 15, 23, 30, 37, 44, 51, 58, 59, 52, 45, 38, 31, 39, 46, 53, 60, 61, 54, 47, 55, 62, 63, };
	...
	/*
	 * Initialisation of the Huffman codes for Luminance and Chrominance. This code results in the same
	 * tables created in the IJG Jpeg-6a library.
	 */
	...
	private void initHuf() {
		...
		/*
		 * Init of the AC hufmann code for the chrominance matrix [][][0] is the code & matrix[][][1] is the
		...
		/*
		 * Init of the AC hufmann code for luminance matrix [][][0] is the code & matrix[][][1] is the
		 * number of bit
		 */
