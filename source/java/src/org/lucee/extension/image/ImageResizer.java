...
public class ImageResizer	{
    ...
    public static BufferedImage resize(BufferedImage image, int columns, int rows, int interpolation, double blur) throws PageException {
    	...
    	if (columns == 0 || rows == 0)
    		throw eng.getExceptionUtil().createExpressionException("invalid size for image");
    ...
    private static Interpolation getInterpolation(int interpolation) throws PageException {
		switch (interpolation) {
			...
			default:	throw CFMLEngineFactory.getInstance().getExceptionUtil().createExpressionException("invalid interpolation definition");
		}
    }
