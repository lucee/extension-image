...
public class ImageResizer {
	...
	private static int horizontal(BufferedImage source, BufferedImage destination, double xFactor, Interpolation ip, double blur, ContributionInfo[] contribution) {
		...
		for (int x = 0; x < destination.getWidth(); x++) {
			...
			int end = (int) Math.min(center + support + 0.5, source.getWidth());
			int n = setContributionWeight(start, end, contribution, ip, center, scale);
			...
			for (int y = 0; y < destination.getHeight(); y++) {
				...
				green = green < 0.0 ? 0.0 : green > 255.0 ? 255.0 : green + 0.5;
	...
	private static int setContributionWeight(int start, int end, ContributionInfo[] contribution, Interpolation interpolation, double center, double scale) {
		...
		for (int i = start; i < end; i++) {
			...
			contribution[n].setWeight(interpolation.f((i - center + 0.5D) / scale) / scale);
	...
	private static int vertical(BufferedImage source, BufferedImage destination, double y_factor, Interpolation ip, double blur, ContributionInfo[] contribution) {
		/*
		 * if (source.getWidth() == destination.getWidth() && source.getHeight() == destination.getHeight())
		 * { destination.setData(source.getData()); return destination.getWidth(); }
		 */
		...
		for (int y = 0; y < destination.getHeight(); y++) {
			...
			int start = (int) Math.max(center - support + 0.5, 0.0);
			int end = (int) Math.min(center + support + 0.5, source.getHeight());
			int n = setContributionWeight(start, end, contribution, ip, center, scale);
	...
	public static BufferedImage resize(BufferedImage image, int columns, int rows, int interpolation, double blur) throws PageException {
		...
		if (columns == 0 || rows == 0) throw eng.getExceptionUtil().createExpressionException("invalid size for image (columns:" + columns + ";rows:" + rows + ")");
		...
		if (support < inter.getSupport()) support = inter.getSupport();
		...
		if (status == 0) throw CFMLEngineFactory.getInstance().getExceptionUtil().createExpressionException("can't resize image");
		return resizeImage;
	}
