...
public class Image extends StructSupport implements Cloneable,Struct {
	...
	public void setDrawingStroke(Struct attr) throws PageException {
    	...
    	else throw CFMLEngineFactory.getInstance().getExceptionUtil().createExpressionException("key [endcaps] has an invalid value ["+strEndcaps+"], valid values are [square,round,butt]");
    	...
    	else throw CFMLEngineFactory.getInstance().getExceptionUtil().createExpressionException("key [linejoins] has an invalid value ["+strLinejoins+"], valid values are [bevel,miter,round]");
	...
	private void _writeOut(ImageOutputStream ios, String format,float quality,boolean noMeta) throws IOException, PageException {
    	...
    	if (writer == null) throw new IOException("no writer for format ["+format+"] available, available writer formats are ["+eng().getListUtil().toList(ImageUtil.getWriterFormatNames(), ",")+"]");
    ...
    public void resize(int scale, String interpolation, double blurFactor) throws PageException {
    	if (blurFactor < 0 || blurFactor > 10.0)
	...
	private float resizeDimesion(String label,String strDimension, float originalDimension) throws PageException {
		...
		if (dimension <= 0F)
			throw CFMLEngineFactory.getInstance().getExceptionUtil().createExpressionException(label+" has to be a none negative number");
		return dimension;
	}
    ...
    /*private BufferedImage resizeImageWithJAI(float scaleWidth, float scaleHeight, int interpolation) throws PageException {
    	ParameterBlock params = new ParameterBlock();
    	params.addSource(image());
    ...
    private double toScale(int src, int dst) {
    	double tmp = Math.round((int)((eng().getCastUtil().toDoubleValue(dst)/eng().getCastUtil().toDoubleValue(src))*100D));
    	return tmp/100D;
	}
	...
	public Object call(PageContext pc, Key methodName, Object[] args) throws PageException {
		...
		/*if(methodName.equals("blur")) {
			new ImageBlur().invoke(pc, pre(args));
			return this;
		}*/
	...
	public static Image createImage(PageContext pc,Object obj, boolean check4Var, boolean clone, boolean checkAccess, String format) throws PageException {
		try {
			...
			if(obj instanceof CharSequence) {
				...
				// file
				try {
					Resource res = eng().getCastUtil().toResource(str);
					pc.getConfig().getSecurityManager().checkFileLocation(res);
					return new Image(res,format);
				} 
