 ...
 * (<A HREF="http://www.dcs.ed.ac.uk/%7Emxr/gfx/">Graphics Interchange Format</A>). A GIFEncoder is
 ...
 * time, depending on the size of the image. Also, actually writing the image out (Write) will take
 * time.
 * <P>
 *
 * <LI>The image cannot have more than 256 colors, since GIF is an 8 bit format. For a 24 bit to 8
 * bit quantization algorithm, see Graphics Gems II III.2 by Xialoin Wu. Or check out his
 ...
 * large images. Attempting to encode an image which will not fit into memory will probably result
...
public class GifEncoder {
	...
	/**
	 * Construct a GIFEncoder. The constructor will convert the image to an indexed color array. <B>This
	 * may take some time.</B>
	 * <P>
	 * 
	 * @param image The image to encode. The image <B>must</B> be completely loaded.
	 * @exception AWTException Will be thrown if the pixel grab fails. This can happen if Java runs out
	 *                of memory. It may also indicate that the image contains more than 256 colors.
	 */
	public GifEncoder(Image image) throws AWTException {
		...
		try {
			if (grabber.grabPixels() != true) throw new AWTException("Grabber returned false: " + grabber.status());
		}
	 ...
	 * <P>
	 *
	 * Each array stores intensity values for the image. In other words, r[x][y] refers to the red
		...
		for (int x = 0; x < width_; ++x) {
			for (int y = 0; y < height_; ++y) {
				...
				for (search = 0; search < colornum; ++search)
					if (colors_[search * 3] == r[x][y] && colors_[search * 3 + 1] == g[x][y] && colors_[search * 3 + 2] == b[x][y]) break;
