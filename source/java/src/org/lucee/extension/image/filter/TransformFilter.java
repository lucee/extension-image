...
public abstract class TransformFilter extends AbstractBufferedImageOp  implements DynFiltering {
     ...
     */
	public void setEdgeAction(String edgeAction) throws PageException {
		...
		if("ZERO".equals(str)) this.edgeAction = ConvolveFilter.ZERO_EDGES;
			...
			throw CFMLEngineFactory.getInstance().getExceptionUtil().createExpressionException("invalid value ["+edgeAction+"] for edgeAction, valid values are [clamp,wrap,zero]");
	}
     ...
     */
	public void setInterpolation(String interpolation) throws PageException {
		...
		if("NEAREST_NEIGHBOUR".equals(str)) this.interpolation = NEAREST_NEIGHBOUR;
			...
			throw CFMLEngineFactory.getInstance().getExceptionUtil().createExpressionException("invalid value ["+interpolation+"] for interpolation, valid values are [bilinear,nearest_neighbour]");
	}
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [EdgeAction, Interpolation]",null);
		}
