...
import org.lucee.extension.image.math.BlackFunction;
...
public abstract class BinaryFilter extends WholeImageFilter  implements DynFiltering {
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("NewColor")))!=null)setNewColor(ImageFilterUtil.toIntValue(o,"NewColor"));
		
		// check for arguments not supported
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, 
					"parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [Iterations, Colormap, NewColor, BlackFunction]",null);
		}
