...
public class CropFilter extends AbstractBufferedImageOp  implements DynFiltering {
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("X")))!=null)setX(ImageFilterUtil.toIntValue(o,"X"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Y")))!=null)setY(ImageFilterUtil.toIntValue(o,"Y"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Width")))!=null)setWidth(ImageFilterUtil.toIntValue(o,"Width"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Height")))!=null)setHeight(ImageFilterUtil.toIntValue(o,"Height"));

		// check for arguments not supported
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [X, Y, Width, Height]",null);
		}
