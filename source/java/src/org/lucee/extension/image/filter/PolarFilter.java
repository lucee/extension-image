...
public class PolarFilter extends TransformFilter  implements DynFiltering {
	...
	public void setType(String type) throws PageException {
		...
		if("RECT_TO_POLAR".equals(type)) this.type = RECT_TO_POLAR;
			...
			throw CFMLEngineFactory.getInstance().getExceptionUtil().createExpressionException("inavlid type defintion ["+type+"], valid types are [RECT_TO_POLAR,POLAR_TO_RECT,INVERT_IN_CIRCLE]");
	}
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=null;//ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Type")))!=null)setType(ImageFilterUtil.toString(o,"Type"));
		...
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [Type, EdgeAction, Interpolation]",null);
		}
