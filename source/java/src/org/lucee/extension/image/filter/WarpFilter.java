...
public class WarpFilter extends WholeImageFilter  implements DynFiltering {
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("SourceGrid")))!=null)setSourceGrid(ImageFilterUtil.toWarpGrid(o,"SourceGrid"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("DestGrid")))!=null)setDestGrid(ImageFilterUtil.toWarpGrid(o,"DestGrid"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Frames")))!=null)setFrames(ImageFilterUtil.toIntValue(o,"Frames"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("MorphImage")))!=null)setMorphImage(ImageFilterUtil.toBufferedImage(o,"MorphImage"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Time")))!=null)setTime(ImageFilterUtil.toFloatValue(o,"Time"));

		// check for arguments not supported
		if(parameters.size()>0) {
			throw CFMLEngineFactory.getInstance().getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [SourceGrid, DestGrid, Frames, MorphImage, Time]",null);
		}
