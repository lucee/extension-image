...
package org.lucee.extension.image.filter;import java.awt.Point;
...
public class GradientFilter extends AbstractBufferedImageOp  implements DynFiltering {
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Interpolation")))!=null)setInterpolation(ImageFilterUtil.toIntValue(o,"Interpolation"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Angle")))!=null)setAngle(ImageFilterUtil.toFloatValue(o,"Angle"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Point1")))!=null)setPoint1(ImageFilterUtil.toPoint(o,"Point1"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Point2")))!=null)setPoint2(ImageFilterUtil.toPoint(o,"Point2"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("PaintMode")))!=null)setPaintMode(ImageFilterUtil.toIntValue(o,"PaintMode"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Type")))!=null)setType(ImageFilterUtil.toIntValue(o,"Type"));

		// check for arguments not supported
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [Colormap, Interpolation, Angle, Point1, Point2, PaintMode, Type]",null);
		}
