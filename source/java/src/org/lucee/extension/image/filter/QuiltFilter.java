...
public class QuiltFilter extends WholeImageFilter  implements DynFiltering {
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("A")))!=null)setA(ImageFilterUtil.toFloatValue(o,"A"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("B")))!=null)setB(ImageFilterUtil.toFloatValue(o,"B"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("C")))!=null)setC(ImageFilterUtil.toFloatValue(o,"C"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("D")))!=null)setD(ImageFilterUtil.toFloatValue(o,"D"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("K")))!=null)setK(ImageFilterUtil.toIntValue(o,"K"));

		// check for arguments not supported
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [Iterations, Colormap, A, B, C, D, K]",null);
		}
