...
public class SmearFilter extends WholeImageFilter  implements DynFiltering {
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Density")))!=null)setDensity(ImageFilterUtil.toFloatValue(o,"Density"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Distance")))!=null)setDistance(ImageFilterUtil.toIntValue(o,"Distance"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Shape")))!=null)setShape(ImageFilterUtil.toIntValue(o,"Shape"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Scatter")))!=null)setScatter(ImageFilterUtil.toFloatValue(o,"Scatter"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Mix")))!=null)setMix(ImageFilterUtil.toFloatValue(o,"Mix"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Fadeout")))!=null)setFadeout(ImageFilterUtil.toIntValue(o,"Fadeout"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Background")))!=null)setBackground(ImageFilterUtil.toBooleanValue(o,"Background"));

		// check for arguments not supported
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [Angle, Density, Distance, Shape, Scatter, Mix, Fadeout, Background]",null);
		}
