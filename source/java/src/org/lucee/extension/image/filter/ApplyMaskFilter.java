...
public class ApplyMaskFilter extends AbstractBufferedImageOp  implements DynFiltering {
	...
	public BufferedImage filter(BufferedImage src ,Struct parameters) throws PageException {
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("MaskImage")))!=null)setMaskImage(ImageFilterUtil.toBufferedImage(o,"MaskImage"));
		...
		if(parameters.size()>0) {
			...
			.createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+
			eng.getListUtil().toList(parameters.keys(), ", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [MaskImage]",null);
		}
