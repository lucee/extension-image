...
import org.lucee.extension.image.composite.AddComposite;
...
public class ShineFilter extends AbstractBufferedImageOp  implements DynFiltering {
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Brightness")))!=null)setBrightness(ImageFilterUtil.toFloatValue(o,"Brightness"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Angle")))!=null)setAngle(ImageFilterUtil.toFloatValue(o,"Angle"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Softness")))!=null)setSoftness(ImageFilterUtil.toFloatValue(o,"Softness"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Distance")))!=null)setDistance(ImageFilterUtil.toFloatValue(o,"Distance"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("ShadowOnly")))!=null)setShadowOnly(ImageFilterUtil.toBooleanValue(o,"ShadowOnly"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Bevel")))!=null)setBevel(ImageFilterUtil.toFloatValue(o,"Bevel"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("ShineColor")))!=null)setShineColor(ImageFilterUtil.toColorRGB(o,"ShineColor"));

		// check for arguments not supported
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [Radius, Brightness, Angle, Softness, Distance, ShadowOnly, Bevel, ShineColor]",null);
		}
