...
public class EmbossFilter extends WholeImageFilter  implements DynFiltering {
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Azimuth")))!=null)setAzimuth(ImageFilterUtil.toFloatValue(o,"Azimuth"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Elevation")))!=null)setElevation(ImageFilterUtil.toFloatValue(o,"Elevation"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Emboss")))!=null)setEmboss(ImageFilterUtil.toBooleanValue(o,"Emboss"));

		// check for arguments not supported
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [BumpHeight, Azimuth, Elevation, Emboss]",null);
		}
