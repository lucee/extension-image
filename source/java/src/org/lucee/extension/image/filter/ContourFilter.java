...
//					outPixels[index] = PixelUtils.combinePixels( (contourColor & 0xff)|(v << 24), inPixels[index], PixelUtils.NORMAL );
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Levels")))!=null)setLevels(ImageFilterUtil.toFloatValue(o,"Levels"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("ContourColor")))!=null)setContourColor(ImageFilterUtil.toColorRGB(o,"ContourColor"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Offset")))!=null)setOffset(ImageFilterUtil.toFloatValue(o,"Offset"));
		...
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [Levels, ContourColor, Offset, Scale]",null);
		}
