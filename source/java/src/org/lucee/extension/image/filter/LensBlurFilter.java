...
//							b = bloomThreshold + (b-bloomThreshold) * bloom;
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Sides")))!=null)setSides(ImageFilterUtil.toIntValue(o,"Sides"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Bloom")))!=null)setBloom(ImageFilterUtil.toFloatValue(o,"Bloom"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("BloomThreshold")))!=null)setBloomThreshold(ImageFilterUtil.toFloatValue(o,"BloomThreshold"));

		// check for arguments not supported
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [Radius, Sides, Bloom, BloomThreshold]",null);
		}
