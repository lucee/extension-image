...
//                   if ( PixelUtils.nearColors( in, clean, (int)(255*tolerance) ) )
	...
	public BufferedImage filter(BufferedImage src, Struct parameters) throws PageException {BufferedImage dst=ImageUtil.createBufferedImage(src);
		...
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("HTolerance")))!=null)setHTolerance(ImageFilterUtil.toFloatValue(o,"HTolerance"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("STolerance")))!=null)setSTolerance(ImageFilterUtil.toFloatValue(o,"STolerance"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("BTolerance")))!=null)setBTolerance(ImageFilterUtil.toFloatValue(o,"BTolerance"));
		if((o=parameters.removeEL(eng.getCreationUtil().createKey("Color")))!=null)setColor(ImageFilterUtil.toIntValue(o,"Color"));

		// check for arguments not supported
		if(parameters.size()>0) {
			throw eng.getExceptionUtil().createFunctionException(eng.getThreadPageContext(), "ImageFilter", 3, "parameters", "the parameter"+(parameters.size()>1?"s":"")+" ["+eng.getListUtil().toList(parameters.keys(),", ")+"] "+(parameters.size()>1?"are":"is")+" not allowed, only the following parameters are supported [HTolerance, STolerance, BTolerance, Color]",null);
		}
