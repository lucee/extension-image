...
public class Img {
	...
	public Img(String text, Font font, int r, int g, int b) {
	...
	// **************************************************************************
	/**
	 * Used to set the background color. Creates an image layer and inserts it under the existing
	 * graphic. This method should only be called once.
	 */
	public void setBackgroundColor(int r, int g, int b) {
		/*
		 * Color org = g2d.getColor(); g2d.setColor(new Color(r,g,b)); g2d.fillRect(1,1,width-2,height-2);
		 * //g2d.fillRect(0,0,width,height); g2d.setColor(org);
		 */
	...
	// **************************************************************************
	/**
	 * Attempts to load classes from the com.sun.image.codec.jpeg package used to compress jpeg images.
	 * These classes are marked as deprecated in Java 1.7 and several distributions of Java no longer
	 * include these classes (e.g. "IcedTea" OpenJDK 7). Returns true of the classes are available.
	 */
	...
	// **************************************************************************
	/**
	 * Used to set the color (ARGB value) for a specific pixel in the image. Note that input x,y values
	...
	// **************************************************************************
	/**
	 * Used to retrieve the color (ARGB) values for a specific pixel in the image. Returns a
	 * java.awt.Color object. Note that input x,y values are relative to the upper left corner of the
	...
	// **************************************************************************
	/**
	 * Used to add an image "overlay" to the existing image at a given position. This method can also be
	 * used to create image mosiacs.
	 */
	 ...
	 * used to create image mosiacs.
	 */
	public void addImage(Img in, int x, int y, boolean expand) {
	...
	// **************************************************************************
	/**
	 * Used to rotate the image (clockwise). Rotation angle is specified in degrees relative to the top
	 * of the image.
	 */
	...
	// **************************************************************************
	/**
	 * Used to resize an image. Provides the option to maintain the original aspect ratio (relative to
	 * the output width).
	 */
	...
	// **************************************************************************
	/**
	 * Used to skew an image by updating the corner coordinates. Coordinates are supplied in clockwise
	...
	// **************************************************************************
	/**
	 * Used to retrieve the corner coordinates of the image. Coordinates are supplied in clockwise order
	 * starting from the upper left corner. This information is particularly useful for generating drop
	 * shadows, inner and outer glow, and reflections. NOTE: Coordinates are not updated after resize(),
	 * rotate(), or addImage()
	 */
	...
	// **************************************************************************
	/**
	 * Used to desaturate an image by a specified percentage (expressed as a double or float). The
	 * larger the percentage, the greater the desaturation and the "grayer" the image. Valid ranges are
	 * from 0-1.
	 */
	...
	// **************************************************************************
	/**
	 * Used to flip an image along it's y-axis (horizontal). Vertical flipping is supported via the
	 * rotate method (i.e. rotate +/-180).
	 */
	public void flip() {
	...
	// **************************************************************************
	/**
	 * Returns a copy of the image at a given rectangle. In Java 1.6, the BufferedImage.getSubimage()
	 * throws an exception if the rectangle falls outside the image bounds. This method was written to
	...
	// **************************************************************************
	/**
	 * Used to remove excess pixels around an image by cropping the image to its "true" extents. Crop
	 * bounds are determined by finding the first non-null or non-black pixel on each side of the image.
	 */
	public void trim() {
		trim(0, 0, 0);
	}
	...
	/**
	 * Used to remove excess pixels around an image by cropping the image to its "true" extents. Crop
	 * bounds are determined by finding pixels that *don't* match the input color. For example, you can
	 * trim off excess black pixels around an image by specifying an rgb value of 0,0,0. Similarly, you
	 * can trim off pure white pixels around an image by specifying an rgb value of 255,255,255. Note
	 * that transparent pixels are considered as null values and will be automatically trimmed from the
	 * edges.
	 */
	...
	public BufferedImage getBufferedImage(int width, int height, boolean maintainRatio) {
		Img image = new Img(getBufferedImage());
	...
	/*
	 * public void setCacheDirectory(java.io.File cacheDirectory){ try{ if (cacheDirectory.isFile()){
	 * cacheDirectory = cacheDirectory.getParentFile(); } cacheDirectory.mkdirs();
	 * ImageIO.setUseCache(true); this.cacheDirectory = cacheDirectory; } catch(Exception e){
	...
	// **************************************************************************
	/**
	 * Used to set the output quality/compression ratio. Only applies when creating JPEG images. Applied
	 * only when writing the image to a file or byte array. Accepts values between 0-1. Also accepts
	 * values between 1-100. If the latter, it divides the value by 100.
	 */
	...
	// **************************************************************************
	/**
	 * Used to determine whether a given pixel has a color value. Returns false if the pixel matches the
	 * input color or is transparent.
	 */
	...
	// **************************************************************************
	/**
	 * Returns the raw, javax.imageio.metadata.IIOMetadata associated with this image. You can iterate
	 * through the metadata using an xml parser like this:
	 * 
	 * <pre>
	...
	// **************************************************************************
	/**
	 * Used to parse IPTC metadata and return a list of key/value pairs found in the metadata. You can
	 * retrieve specific IPTC metadata values like this:
	 * 
	 * <pre>
	...
	// **************************************************************************
	/**
	 * Used to parse EXIF metadata and return a list of key/value pairs found in the metadata. Values
	 * can be Strings, Integers, or raw Byte Arrays. You can retrieve specific EXIF metadata values like
	 * this:
	 * 
	 * <pre>
	 * javaxt.io.Image image = new javaxt.io.Image("/temp/image.jpg");
	...
	// **************************************************************************
	/**
	 * Used to parse EXIF metadata and return a list of key/value pairs associated with GPS metadata.
	 * Values can be Strings, Integers, or raw Byte Arrays.
	 */
	...
	// **************************************************************************
	/**
	 * Returns the x/y (lon/lat) coordinate tuple for the image. Value is derived from EXIF GPS metadata
	...
	// **************************************************************************
	/**
	 * Returns the datum associated with the GPS coordinate. Value is derived from EXIF GPS metadata
	 * (tag 0x0012).
	 */
	...
	// **************************************************************************
	/**
	 * Returns a list of "unknown" IIOMetadataNodes for a given MarkerTag. You can use this method to
	 * retrieve EXIF, IPTC, XPM, and other format specific metadata. Example:
	 * 
	 * <pre>
	...
	public IIOMetadataNode[] getUnknownTags(int MarkerTag) {
		...
		if (metadata != null) for (String name: metadata.getMetadataFormatNames()) {
			IIOMetadataNode node = (IIOMetadataNode) metadata.getAsTree(name);
	...
	// **************************************************************************
	/**
	 * Returns a list of IIOMetadataNodes for a given tag name (e.g. "Chroma", "Compression", "Data",
	...
	// **************************************************************************
	/**
	 * Returns an array of nodes that match a given tagName (node name). The results will include all
	 * nodes that match, regardless of namespace. To narrow the results to a specific namespace, simply
	 * include the namespace prefix in the tag name (e.g. "t:Contact"). Returns an empty array if no
	 * nodes are found.
	 */
	...
	// **************************************************************************
	/**
	 * Used to return the value of a given node attribute. The search is case insensitive. If no match
	...
	// ******************************************************************************
	/**
	 * Used to decode EXIF and IPTC metadata. Adapted from 2 classes developed by Norman Walsh and
	 * released under the W3C open source license. The original exif classes can be found in the W3C
	 * Jigsaw project in the org.w3c.tools.jpeg package.
	 *
	 * @author Norman Walsh
	...
	private class MetadataParser {
		...
		// **************************************************************************
		/**
		 * Returns a string representation of a rational number (numerator and denominator separated with a
		 * "/" character).
		 */
		...
		// **************************************************************************
		/**
		 * Used convert a byte into a double. Note that this method used to be called convertAnyValue().
		 */
	...
	private class Skew {
		 ...
		 * src.getHeight()); }
		 * 
		 * public Point2D getPoint2D( Point2D srcPt, Point2D dstPt ) { if ( dstPt == null ) dstPt = new
		 * Point2D.Double(); dstPt.setLocation( srcPt.getX(), srcPt.getY() ); return dstPt; }
		 */
